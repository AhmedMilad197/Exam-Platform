  ...
  <div>
    <v-locale-provider rtl>
        ...
        <v-toolbar>
          ...
          <div class="d-flex mx-4 my-4">
            <v-btn color="white" class="pink-darken"
              @click="navigateTo({ name: 'subject-teacher', params: { course: route.params.id } })">إضافة أستاذ</v-btn>
            <v-btn color="white" class="mx-4 primary" @click="goBack()">العودة</v-btn>
          </div>
        </v-toolbar>
        <template v-slot:text>
          <v-text-field v-model="search" label="إبحث عن الاساتذة" hide-details />
        </template>
        <v-data-table :headers="headers" :items="teachers" :search="search">
          <template v-slot:item.actions="{ item }">
            <v-icon size="small" color="red" @click="removeTeacherConfrimation(item)">
              mdi-delete
            </v-icon>
          </template>
        </v-data-table>
      ...
      <v-dialog v-model="dialog" width="auto">
        <v-card max-width="400" prepend-icon="mdi-alert-circle" text="هل تريد إزالة هذا الأستاذ من المادة الحالية؟"
          title="تأكيد" color="orange">
          <v-card>
